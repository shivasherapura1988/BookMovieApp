...
class Home extends Component {
	...
	componentWillMount() {
		...
		// http calls to get movies which are published.

		xhr.open("GET", this.props.baseUrl + "movies?status=PUBLISHED");
		xhr.setRequestHeader("Cache-Control", "no-cache");
		xhr.send(data);

		// Get released movie which are ready.
		...
		// Get filters which are selected
	...
	filterApplyHandler = () => {
		...
		//Get the movies which are selected for query.
			...
			className = {
				...
				} >
				{
					this.state.genresList.map(genre => ( <
						...
						<
						Checkbox checked = {
							this.state.genres.indexOf(genre.genre) > -1
						}
						/> <
				...
				} >
				{
					this.state.artistsList.map(artist => ( <
						...
						<
						Checkbox checked = {
							this.state.artists.indexOf(artist.first_name + " " + artist.last_name) > -1
						}
						/> <
						ListItemText primary = {
							artist.first_name + " " + artist.last_name
